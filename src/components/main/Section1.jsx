    ...
    <section className="w-full flex justify-center mt-20 px-4">
      <div className="max-w-337.5 w-full flex flex-wrap gap-8">
        ...
        <div className="w-full lg:w-[65%] flex flex-wrap gap-6">
          {[{img: frame9, title:"Natural Plant", price:"$1,400.00"},
            {img: frame8, title:"Artificial Plant", price:"$900.00"},
            {img: frame7, title:"Premium Plant", price:"$3,500.00"}].map((item, i) => (
            <div key={i} className="w-full sm:w-[48%] lg:w-[31%] shadow-md rounded-2xl p-6 text-center hover:shadow-xl transition duration-300">
              <img src={item.img} alt={item.title} className="mx-auto w-full h-60 sm:h-64 lg:h-60 object-contain"/>
              <h6 className="font-semibold text-lg mt-4">{item.title}</h6>
