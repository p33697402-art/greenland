...
export const Section2 = () => {
  const items = [
    ...
    {img: f2, title:"Plant Accessories", desc:"Horem ipsum dolor sit amet, consectetur adipiscing elit."},
  ...
  return (
    <section className="w-full bg-[#aadcdc] flex justify-center py-16 mt-32">
      <div className="w-[80%] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {items.map((item, i) => (
          <div key={i} className="flex justify-center flex-col items-center hover:scale-105 transform transition duration-300">
            <img src={item.img} alt={item.title} className="w-[80%] block"/>
            <h2 className="font-bold">{item.title}</h2>
            {item.desc && <p className="text-black-300 text-center">{item.desc}</p>}
